...
export default {
  ...
  methods: {
    ...
    approve: async function (_price) {
      const cUSDContract = new this.kit.web3.eth.Contract(
        erc20Abi,
        cUSDContractAddress
      );
    ...
    supportCampaign: async function (index) {
      Swal.fire({
        ...
        preConfirm: async (amount) => {
          ...
          Swal.fire("Thank you!", "You just saved a live!", "success");
        },
      });
    },
    ...
    fetchCampaignMeta: async function (_index) {
      const campaignMeta = await this.contract.methods
        .fetchCampaignMeta(_index)
        .call();
    ...
    getWalletBalance: async function () {
      const totalBalance = await this.kit.getTotalBalance(this.defaultAccount);
